...
import { archiveProject } from "../../../domain/project/ProjectRules.js";
...
import type { ArchiveProjectCommand } from "../../ports/inbound/commands/ArchiveProject.js";
...
import type { ProjectArchived } from "../../../domain/project/ProjectEvents.js";

export type ArchiveProjectError = ProjectStateError | NotFoundError;
...
export class ArchiveProjectHandler {
  ...
  async execute(cmd: ArchiveProjectCommand, ctx: RequestContext): Promise<Result<ProjectDTO, ArchiveProjectError>> {
    ...
    const result = archiveProject(existing, now);
    if (!result.ok) return result;

    await this.projectRepo.save(result.value);

    const event: ProjectArchived = {
